...
def dissipation_on_wgrid(vs, out, int_drhodX=None, aloc=None, ks=None):
    if aloc is None:
        ...
        aloc = jax.ops.index_update(aloc, jax.ops.index[1:-1, 1:-1, :],
            0.5 * vs.grav / vs.rho_0 \
    ...
    out = jax.ops.index_add(out, jax.ops.index[:, :, :-1],
        (0.5 * (aloc[:, :, :-1] + aloc[:, :, 1:])
        + 0.5 * (aloc[:, :, :-1] * dzw_pad[np.newaxis, np.newaxis, :-3]
                / vs.dzw[np.newaxis, np.newaxis, :-1])) * edge_mask
    )
    out = jax.ops.index_add(out, jax.ops.index[:, :, :-1],
        0.5 * (aloc[:, :, :-1] + aloc[:, :, 1:]) * water_mask)
    out = jax.ops.index_add(out, jax.ops.index[:, :, -1], aloc[:, :, -1] * land_mask)
...
def tempsalt_biharmonic(vs):
    ...
    vs.flux_east = jax.ops.index_update(vs.flux_east, jax.ops.index[:-1, :, :],
        -fxa * (vs.temp[1:, :, :, vs.tau] - vs.temp[:-1, :, :, vs.tau]) \
    ...
    del2 = jax.ops.index_update(del2, jax.ops.index[1:, 1:, :],
        vs.maskT[1:, 1:, :] * (vs.flux_east[1:, 1:, :] - vs.flux_east[:-1, 1:, :]) \
    ...
    # update tendency
    vs.dtemp_hmix = jax.ops.index_update(vs.dtemp_hmix, jax.ops.index[1:, 1:, :],
        vs.maskT[1:, 1:, :] * (vs.flux_east[1:, 1:, :] - vs.flux_east[:-1, 1:, :]) \
        / (vs.cost[np.newaxis, 1:, np.newaxis] * vs.dxt[1:, np.newaxis, np.newaxis]) \
        + (vs.flux_north[1:, 1:, :] - vs.flux_north[1:, :-1, :]) \
        / (vs.cost[np.newaxis, 1:, np.newaxis] * vs.dyt[np.newaxis, 1:, np.newaxis]))
    vs.temp = jax.ops.index_add(vs.temp, jax.ops.index[:, :, :, vs.taup1],
        vs.dt_tracer * vs.dtemp_hmix * vs.maskT)

    if vs.enable_conserve_energy:
        if vs.pyom_compatibility_mode:
            fxa = vs.int_drhodT[-3, -3, -1, vs.tau]
        vs.P_diss_hmix = jax.ops.index_update(vs.P_diss_hmix, jax.ops.index[...], 0.)
        dissipation_on_wgrid(vs, vs.P_diss_hmix, int_drhodX=vs.int_drhodT[..., vs.tau])

    vs.flux_east = jax.ops.index_update(vs.flux_east, jax.ops.index[:-1, :, :],
        -fxa * (vs.salt[1:, :, :, vs.tau] - vs.salt[:-1, :, :, vs.tau]) \
        / (vs.cost[np.newaxis, :, np.newaxis] * vs.dxu[:-1, np.newaxis, np.newaxis]) * vs.maskU[:-1, :, :])
    vs.flux_north = jax.ops.index_update(vs.flux_north, jax.ops.index[:, :-1, :],
        -fxa * (vs.salt[:, 1:, :, vs.tau] - vs.salt[:, :-1, :, vs.tau]) \
        / vs.dyu[np.newaxis, :-1, np.newaxis] * vs.maskV[:, :-1, :] * vs.cosu[np.newaxis, :-1, np.newaxis])
    ...
    # update tendency
    vs.dsalt_hmix = jax.ops.index_update(vs.dsalt_hmix, jax.ops.index[1:, 1:, :],
        vs.maskT[1:, 1:, :] * (vs.flux_east[1:, 1:, :] - vs.flux_east[:-1, 1:, :]) \
        / (vs.cost[np.newaxis, 1:, np.newaxis] * vs.dxt[1:, np.newaxis, np.newaxis]) \
        + (vs.flux_north[1:, 1:, :] - vs.flux_north[1:, :-1, :]) \
        / (vs.cost[np.newaxis, 1:, np.newaxis] * vs.dyt[np.newaxis, 1:, np.newaxis])
    )
    vs.salt = jax.ops.index_add(vs.salt, jax.ops.index[:, :, :, vs.taup1],
        vs.dt_tracer * vs.dsalt_hmix * vs.maskT)
...
def tempsalt_diffusion(vs):
    ...
    vs.flux_east = jax.ops.index_update(vs.flux_east, jax.ops.index[:-1, :, :],
        vs.K_h * (vs.temp[1:, :, :, vs.tau] - vs.temp[:-1, :, :, vs.tau]) \
        / (vs.cost[np.newaxis, :, np.newaxis] * vs.dxu[:-1, np.newaxis, np.newaxis]) * vs.maskU[:-1, :, :]
    )
    vs.flux_east = jax.ops.index_update(vs.flux_east, jax.ops.index[-1, :, :],
        0.)
    ...
    vs.flux_east = jax.ops.index_update(vs.flux_east, jax.ops.index[:-1, :, :],
        vs.K_h * (vs.salt[1:, :, :, vs.tau] - vs.salt[:-1, :, :, vs.tau]) \
        / (vs.cost[np.newaxis, :, np.newaxis] * vs.dxu[:-1, np.newaxis, np.newaxis]) * vs.maskU[:-1, :, :])
    vs.flux_east = jax.ops.index_update(vs.flux_east, jax.ops.index[-1, :, :], 0.)
...
def tempsalt_sources(vs):
    ...
    """
    vs.temp = jax.ops.index_add(vs.temp, jax.ops.index[:, :, :, vs.taup1], vs.dt_tracer * vs.temp_source * vs.maskT)
    vs.salt = jax.ops.index_add(vs.salt, jax.ops.index[:, :, :, vs.taup1], vs.dt_tracer * vs.salt_source * vs.maskT)

    if vs.enable_conserve_energy:
        aloc = -vs.grav / vs.rho_0 * vs.maskT * \
            (vs.int_drhodT[..., vs.tau] * vs.temp_source +
             vs.int_drhodS[..., vs.tau] * vs.salt_source)
        vs.P_diss_sources = jax.ops.index_update(vs.P_diss_sources, jax.ops.index[...], 0.)
