...
def init_eke(vs):
    ...
    if vs.enable_eke_leewave_dissipation:
        vs.hrms_k0 = jax.ops.index_update(vs.hrms_k0, jax.ops.index[...],
            np.maximum(vs.eke_hrms_k0_min, 2 / vs.pi * vs.eke_topo_hrms**2
...
def set_eke_diffusivities(vs):
    ...
    if vs.enable_eke:
        ...
        """
        C_rossby = jax.ops.index_update(C_rossby, jax.ops.index[...],
            np.sum(np.sqrt(np.maximum(0., vs.Nsqr[:, :, :, vs.tau]))
                                * vs.dzw[np.newaxis, np.newaxis, :] * vs.maskW[:, :, :] / vs.pi, axis=2))
        vs.L_rossby = jax.ops.index_update(vs.L_rossby, jax.ops.index[...],
            np.minimum(C_rossby / np.maximum(np.abs(vs.coriolis_t), 1e-16),
        ...
        vs.L_rhines = jax.ops.index_update(vs.L_rhines, jax.ops.index[...],
            np.sqrt(vs.sqrteke / np.maximum(vs.beta[..., np.newaxis], 1e-16))
        )
        vs.eke_len = jax.ops.index_update(vs.eke_len, jax.ops.index[...],
            np.maximum(vs.eke_lmin, np.minimum(
                vs.eke_cross * vs.L_rossby[..., np.newaxis], vs.eke_crhin * vs.L_rhines))
        )
        vs.K_gm = jax.ops.index_update(vs.K_gm, jax.ops.index[...],
            np.minimum(vs.eke_k_max, vs.eke_c_k * vs.eke_len * vs.sqrteke)
        )
    else:
        """
        use fixed GM diffusivity
        """
        vs.K_gm = jax.ops.index_update(vs.K_gm, jax.ops.index[...], vs.K_gm_0)

    if vs.enable_TEM_friction:
        vs.kappa_gm = jax.ops.index_update(vs.kappa_gm, jax.ops.index[...],
            vs.K_gm * np.minimum(0.01, vs.coriolis_t[..., np.newaxis]**2
            / np.maximum(1e-9, vs.Nsqr[..., vs.tau])) * vs.maskW
        )
    if vs.enable_eke and vs.enable_eke_isopycnal_diffusion:
        vs.K_iso = jax.ops.index_update(vs.K_iso, jax.ops.index[...], vs.K_gm)
    else:
        vs.K_iso = jax.ops.index_update(vs.K_iso, jax.ops.index[...], vs.K_iso_0)  # always constant
...
def integrate_eke(vs):
    ...
    if vs.enable_eke_leewave_dissipation:
        ...
        """
        vs.c_lee = jax.ops.index_update(vs.c_lee, jax.ops.index[...], 0.)
        ...
        vs.c_lee = jax.ops.index_update(vs.c_lee, jax.ops.index[2:-2, 2:-2],
            boundary_mask * vs.c_lee0 * vs.hrms_k0[2:-2, 2:-2] \
        ...
        """
        vs.c_Ri_diss = jax.ops.index_update(vs.c_Ri_diss, jax.ops.index[...], 0)
        ...
        vs.c_Ri_diss = jax.ops.index_update(vs.c_Ri_diss, jax.ops.index[1:, 1:, :-1],
            vs.maskW[1:, 1:, :-1] * fxa * vs.eke_int_diss0)
        vs.c_Ri_diss = jax.ops.index_update(vs.c_Ri_diss, jax.ops.index[:, :, -1],
            vs.c_Ri_diss[:, :, -2] * vs.maskW[:, :, -1])
        ...
        """
        a_loc = jax.ops.index_add(a_loc, jax.ops.index[2:-2, 2:-2],
            np.sum((vs.c_lee[2:-2, 2:-2, np.newaxis] * vs.eke[2:-2, 2:-2, :, vs.tau]
        ...
        """
        mask = b_loc > 0
        a_loc = jax.ops.index_update(a_loc, jax.ops.index[...],
            utilities.where(vs, mask, a_loc / (b_loc + 1e-20), 0.))
        c_int = jax.ops.index_update(c_int, jax.ops.index[...],
            a_loc[:, :, np.newaxis])
    else:
        """
        ...
        """
        c_int = jax.ops.index_update(c_int, jax.ops.index[...],
            vs.eke_c_eps * vs.sqrteke / vs.eke_len * vs.maskW)
    ...
    delta = jax.ops.index_update(delta, jax.ops.index[:, :, :-1],
        vs.dt_tracer / vs.dzt[np.newaxis, np.newaxis, 1:] * 0.5 \
        * (vs.kappaM[2:-2, 2:-2, :-1] + vs.kappaM[2:-2, 2:-2, 1:]) * vs.alpha_eke)
    a_tri = jax.ops.index_update(a_tri, jax.ops.index[:, :, 1:-1],
        -delta[:, :, :-2] / vs.dzw[1:-1])
    ...
    c_tri = jax.ops.index_update(c_tri, jax.ops.index[:, :, :-1],
        -delta[:, :, :-1] / vs.dzw[np.newaxis, np.newaxis, :-1])
    d_tri = jax.ops.index_update(d_tri, jax.ops.index[:, :, :],
        vs.eke[2:-2, 2:-2, :, vs.tau] + vs.dt_tracer * forc[2:-2, 2:-2, :])
    sol, water_mask = utilities.solve_implicit(vs, ks, a_tri, b_tri, c_tri, d_tri, b_edge=b_tri_edge)
    vs.eke = jax.ops.index_update(vs.eke, jax.ops.index[2:-2, 2:-2, :, vs.taup1],
        utilities.where(vs, water_mask, sol, vs.eke[2:-2, 2:-2, :, vs.taup1]))

    """
    store eke dissipation
    """
    if vs.enable_eke_leewave_dissipation:
        vs.eke_diss_iw = jax.ops.index_update(vs.eke_diss_iw, jax.ops.index[...], 0.)
        vs.eke_diss_tke = jax.ops.index_update(vs.eke_diss_tke, jax.ops.index[...], vs.c_Ri_diss * vs.eke[:, :, :, vs.taup1])

        """
        flux by lee wave generation and bottom friction
        """
        vs.eke_diss_iw = jax.ops.index_add(vs.eke_diss_iw, jax.ops.index[2:-2, 2:-2, :],
            (vs.c_lee[2:-2, 2:-2, np.newaxis] * vs.eke[2:-2, 2:-2, :, vs.taup1] * vs.maskW[2:-2, 2:-2, :]) * full_mask)
        if vs.pyom_compatibility_mode:
            vs.eke_diss_tke = jax.ops.index_add(vs.eke_diss_tke, jax.ops.index[2:-2, 2:-2, :],
                (2 * vs.eke_r_bot * vs.eke[2:-2, 2:-2, :, vs.taup1] * np.sqrt(np.float32(2.0))
                * vs.sqrteke[2:-2, 2:-2, :] * vs.maskW[2:-2, 2:-2, :] / vs.dzw[np.newaxis, np.newaxis, :]) * full_mask)
        else:
            vs.eke_diss_tke = jax.ops.index_add(vs.eke_diss_tke, jax.ops.index[2:-2, 2:-2, :],
                (2 * vs.eke_r_bot * vs.eke[2:-2, 2:-2, :, vs.taup1] * math.sqrt(2.0)
                * vs.sqrteke[2:-2, 2:-2, :] * vs.maskW[2:-2, 2:-2, :] / vs.dzw[np.newaxis, np.newaxis, :]) * full_mask)
        """
        ...
        mask = a_loc != 0.
        b_loc = jax.ops.index_update(b_loc, jax.ops.index[...], utilities.where(vs, mask, b_loc / (a_loc + 1e-20), 0.))
        ...
        """
        vs.eke_lee_flux = jax.ops.index_update(vs.eke_lee_flux, jax.ops.index[2:-2, 2:-2],
            utilities.where(vs, boundary_mask, np.sum(vs.c_lee[2:-2, 2:-2, np.newaxis] * vs.eke[2:-2, 2:-2, :, vs.taup1]
            * vs.dzw[np.newaxis, np.newaxis, :] * full_mask, axis=-1), vs.eke_lee_flux[2:-2, 2:-2]))
        vs.eke_bot_flux = jax.ops.index_update(vs.eke_bot_flux, jax.ops.index[2:-2, 2:-2],
            utilities.where(vs, boundary_mask, np.sum(2 * vs.eke_r_bot * vs.eke[2:-2, 2:-2, :, vs.taup1]
            * math.sqrt(2.0) * vs.sqrteke[2:-2, 2:-2, :] * full_mask, axis=-1), vs.eke_bot_flux[2:-2, 2:-2]))
    else:
        vs.eke_diss_iw = c_int * vs.eke[:, :, :, vs.taup1]
        vs.eke_diss_tke = jax.ops.index_update(vs.eke_diss_tke, jax.ops.index[...], 0.)

    """
    add tendency due to lateral diffusion
    """
    vs.flux_east = jax.ops.index_update(vs.flux_east, jax.ops.index[:-1, :, :],
        0.5 * np.maximum(500., vs.K_gm[:-1, :, :] + vs.K_gm[1:, :, :]) \
    ...
    vs.flux_north = jax.ops.index_update(vs.flux_north, jax.ops.index[:, -1, :],
        0.)
    vs.eke = jax.ops.index_add(vs.eke, jax.ops.index[2:-2, 2:-2, :, vs.taup1],
        vs.dt_tracer * vs.maskW[2:-2, 2:-2, :]
    ...
    if vs.enable_eke_superbee_advection or vs.enable_eke_upwind_advection:
        vs.deke = jax.ops.index_update(vs.deke, jax.ops.index[2:-2, 2:-2, :, vs.tau],
            vs.maskW[2:-2, 2:-2, :] * (-(vs.flux_east[2:-2, 2:-2, :] - vs.flux_east[1:-3, 2:-2, :])
            / (vs.cost[np.newaxis, 2:-2, np.newaxis] * vs.dxt[2:-2, np.newaxis, np.newaxis])
            - (vs.flux_north[2:-2, 2:-2, :] - vs.flux_north[2:-2, 1:-3, :])
            / (vs.cost[np.newaxis, 2:-2, np.newaxis] * vs.dyt[np.newaxis, 2:-2, np.newaxis]))
        )
        vs.deke = jax.ops.index_add(vs.deke, jax.ops.index[:, :, 0, vs.tau], -vs.flux_top[:, :, 0] / vs.dzw[0])
        vs.deke = jax.ops.index_add(vs.deke, jax.ops.index[:, :, 1:-1, vs.tau],
            -(vs.flux_top[:, :, 1:-1] - vs.flux_top[:, :, :-2]) / vs.dzw[np.newaxis, np.newaxis, 1:-1])
        vs.deke = jax.ops.index_add(vs.deke, jax.ops.index[:, :, -1, vs.tau],
            -(vs.flux_top[:, :, -1] - vs.flux_top[:, :, -2]) / (0.5 * vs.dzw[-1]))
        """
        Adam Bashforth time stepping
        """
        vs.eke = jax.ops.index_add(vs.eke, jax.ops.index[:, :, :, vs.taup1],
            vs.dt_tracer * ((1.5 + vs.AB_eps) * vs.deke[:, :, :, vs.tau] - (0.5 + vs.AB_eps) * vs.deke[:, :, :, vs.taum1]))
