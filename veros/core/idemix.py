...
def set_idemix_parameter(vs):
    ...
    vs.c0 = jax.ops.index_update(vs.c0, jax.ops.index[...],
        np.maximum(0., vs.gamma * cstar * gofx2(vs, fxa) * vs.maskW))
    vs.v0 = jax.ops.index_update(vs.v0, jax.ops.index[...],
        np.maximum(0., vs.gamma * cstar * hofx1(vs, fxa) * vs.maskW))
    vs.alpha_c = jax.ops.index_update(vs.alpha_c, jax.ops.index[...],
        np.maximum(1e-4, vs.mu0 * np.arccosh(np.maximum(1., fxa))
...
def integrate_idemix(vs):
    ...
    """
    if vs.enable_eke:
        forc = jax.ops.index_update(forc, jax.ops.index[...], vs.eke_diss_iw)
    else:  # shortcut without EKE model
        if vs.enable_store_cabbeling_heat:
            forc = jax.ops.index_update(forc, jax.ops.index[...],
                vs.K_diss_gm + vs.K_diss_h - \
                vs.P_diss_skew - vs.P_diss_hmix - vs.P_diss_iso
            )
        else:
            forc = jax.ops.index_update(forc, jax.ops.index[...],
                vs.K_diss_gm + vs.K_diss_h - vs.P_diss_skew)
    ...
    if vs.enable_eke and (vs.enable_eke_diss_bottom or vs.enable_eke_diss_surfbot):
        ...
        forc = jax.ops.index_update(forc, jax.ops.index[...], 0.)

        ks = np.maximum(0, vs.kbot[2:-2, 2:-2] - 1)
        mask = ks[:, :, np.newaxis] == np.arange(vs.nz)[np.newaxis, np.newaxis, :]
        if vs.enable_eke_diss_bottom:
            forc = jax.ops.index_update(forc, jax.ops.index[2:-2, 2:-2, :],
                utilities.where(vs, mask, a_loc[2:-2, 2:-2, np.newaxis] /
                                           vs.dzw[np.newaxis, np.newaxis, :], forc[2:-2, 2:-2, :]))
        else:
            forc = jax.ops.index_update(forc, jax.ops.index[2:-2, 2:-2, :],
                utilities.where(vs, mask, vs.eke_diss_surfbot_frac * a_loc[2:-2, 2:-2, np.newaxis]
                                           / vs.dzw[np.newaxis, np.newaxis, :], forc[2:-2, 2:-2, :]))
            forc = jax.ops.index_update(forc, jax.ops.index[2:-2, 2:-2, -1],
                (1. - vs.eke_diss_surfbot_frac) \
    ...
    """
    maxE_iw = jax.ops.index_update(maxE_iw, jax.ops.index[...],
        np.maximum(0., vs.E_iw[:, :, :, vs.tau]))
    ...
    delta = jax.ops.index_update(delta, jax.ops.index[:, :, :-1],
        vs.dt_tracer * vs.tau_v / vs.dzt[np.newaxis, np.newaxis, 1:] * 0.5 \
    ...
    b_tri = jax.ops.index_update(b_tri, jax.ops.index[:, :, -1],
        1 + delta[:, :, -2] / (0.5 * vs.dzw[-1:]) * vs.c0[2:-2, 2:-2, -1] \
    ...
    d_tri = jax.ops.index_update(d_tri, jax.ops.index[...],
        vs.E_iw[2:-2, 2:-2, :, vs.tau] + vs.dt_tracer * forc[2:-2, 2:-2, :])
    d_tri_edge = d_tri + vs.dt_tracer * \
        vs.forc_iw_bottom[2:-2, 2:-2, np.newaxis] / vs.dzw[np.newaxis, np.newaxis, :]
    d_tri = jax.ops.index_add(d_tri, jax.ops.index[:, :, -1], vs.dt_tracer * vs.forc_iw_surface[2:-2, 2:-2] / (0.5 * vs.dzw[-1:]))
    sol, water_mask = utilities.solve_implicit(vs, ks, a_tri, b_tri, c_tri, d_tri, b_edge=b_tri_edge, d_edge=d_tri_edge)
    vs.E_iw = jax.ops.index_update(vs.E_iw, jax.ops.index[2:-2, 2:-2, :, vs.taup1],
        utilities.where(vs, water_mask, sol, vs.E_iw[2:-2, 2:-2, :, vs.taup1]))

    """
    store IW dissipation
    """
    vs.iw_diss = jax.ops.index_update(vs.iw_diss, jax.ops.index[...],
        vs.alpha_c * maxE_iw * vs.E_iw[..., vs.taup1])
    ...
    if vs.enable_idemix_hor_diffusion:
        ...
        vs.E_iw = jax.ops.index_add(vs.E_iw, jax.ops.index[2:-2, 2:-2, :, vs.taup1],
    ...
    if vs.enable_idemix_superbee_advection or vs.enable_idemix_upwind_advection:
        vs.dE_iw = jax.ops.index_update(vs.dE_iw, jax.ops.index[2:-2, 2:-2, :, vs.tau],
            vs.maskW[2:-2, 2:-2, :] * (-(vs.flux_east[2:-2, 2:-2, :] - vs.flux_east[1:-3, 2:-2, :])
                                                                    / (vs.cost[np.newaxis, 2:-2, np.newaxis] * vs.dxt[2:-2, np.newaxis, np.newaxis])
                                                                    - (vs.flux_north[2:-2, 2:-2, :] - vs.flux_north[2:-2, 1:-3, :])
                                                                    / (vs.cost[np.newaxis, 2:-2, np.newaxis] * vs.dyt[np.newaxis, 2:-2, np.newaxis]))
        )
        vs.dE_iw = jax.ops.index_add(vs.dE_iw, jax.ops.index[:, :, 0, vs.tau],
            -vs.flux_top[:, :, 0] / vs.dzw[0:1])
        vs.dE_iw = jax.ops.index_add(vs.dE_iw, jax.ops.index[:, :, 1:-1, vs.tau],
            -(vs.flux_top[:, :, 1:-1] - vs.flux_top[:, :, :-2]) \
            / vs.dzw[np.newaxis, np.newaxis, 1:-1]
        )
        vs.dE_iw = jax.ops.index_add(vs.dE_iw, jax.ops.index[:, :, -1, vs.tau],
        ...
        """
        vs.E_iw = jax.ops.index_add(vs.E_iw, jax.ops.index[:, :, :, vs.taup1],
            vs.dt_tracer * ((1.5 + vs.AB_eps) * vs.dE_iw[:, :, :, vs.tau]
