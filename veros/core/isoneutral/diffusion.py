...
def _calc_tracer_fluxes(vs, tr, K_iso, K_skew):
    ...
    diffloc = jax.ops.index_update(diffloc, jax.ops.index[:, :, 1:],
        0.25 * (K1[1:-2, 2:-2, 1:] + K1[1:-2, 2:-2, :-1] +
                                K1[2:-1, 2:-2, 1:] + K1[2:-1, 2:-2, :-1]))
    diffloc = jax.ops.index_update(diffloc, jax.ops.index[:, :, 0],
        0.5 * (K1[1:-2, 2:-2, 0] + K1[2:-1, 2:-2, 0]))
    ...
    vs.flux_east = jax.ops.index_update(vs.flux_east, jax.ops.index[1:-2, 2:-2, :],
        sumz / (4. * vs.dzt[np.newaxis, np.newaxis, :]) + (tr[2:-1, 2:-2, :, vs.tau] - tr[1:-2, 2:-2, :, vs.tau]) \
    ...
    diffloc = jax.ops.index_update(diffloc, jax.ops.index[:, :, 1:],
        0.25 * (K1[2:-2, 1:-2, 1:] + K1[2:-2, 1:-2, :-1] +
                                K1[2:-2, 2:-1, 1:] + K1[2:-2, 2:-1, :-1]))
    diffloc = jax.ops.index_update(diffloc, jax.ops.index[:, :, 0],
        0.5 * (K1[2:-2, 1:-2, 0] + K1[2:-2, 2:-1, 0]))
    ...
    vs.flux_north = jax.ops.index_update(vs.flux_north, jax.ops.index[2:-2, 1:-2, :],
        vs.cosu[np.newaxis, 1:-2, np.newaxis] * (sumz / (4. * vs.dzt[np.newaxis, np.newaxis, :]) \
    ...
    vs.flux_top = jax.ops.index_update(vs.flux_top, jax.ops.index[:, :, -1], 0.)
...
def _calc_explicit_part(vs):
    ...
    aloc = jax.ops.index_update(aloc, jax.ops.index[2:-2, 2:-2, :],
        vs.maskT[2:-2, 2:-2, :] * ((vs.flux_east[2:-2, 2:-2, :] - vs.flux_east[1:-3, 2:-2, :]) / (vs.cost[np.newaxis, 2:-2, np.newaxis] * vs.dxt[2:-2, np.newaxis, np.newaxis])
                                                   + (vs.flux_north[2:-2, 2:-2, :] - vs.flux_north[2:-2, 1:-3, :]) / (vs.cost[np.newaxis, 2:-2, np.newaxis] * vs.dyt[np.newaxis, 2:-2, np.newaxis]))
    )
    aloc = jax.ops.index_add(aloc, jax.ops.index[:, :, 0], vs.maskT[:, :, 0] * vs.flux_top[:, :, 0] / vs.dzt[0])
    aloc = jax.ops.index_add(aloc, jax.ops.index[:, :, 1:], vs.maskT[:, :, 1:] *
...
def _calc_implicit_part(vs, tr):
    ...
    delta = jax.ops.index_update(delta, jax.ops.index[:, :, :-1],
        vs.dt_tracer / vs.dzw[np.newaxis, np.newaxis, :-1] * vs.K_33[2:-2, 2:-2, :-1])
    delta = jax.ops.index_update(delta, jax.ops.index[:, :, -1],
        0.)
    ...
    b_tri = jax.ops.index_update(b_tri, jax.ops.index[:, :, -1],
        1 + delta[:, :, -2] / vs.dzt[np.newaxis, np.newaxis, -1])
    ...
    tr = jax.ops.index_update(tr, jax.ops.index[2:-2, 2:-2, :, vs.taup1],
        utilities.where(vs, water_mask, sol, tr[2:-2, 2:-2, :, vs.taup1]))
...
def isoneutral_diffusion(vs, tr, istemp, iso=True, skew=False):
    ...
    if istemp:
        vs.dtemp_iso = jax.ops.index_add(vs.dtemp_iso, jax.ops.index[...], aloc[...])
    else:
        vs.dsalt_iso = jax.ops.index_add(vs.dsalt_iso, jax.ops.index[...], aloc[...])
    tr = jax.ops.index_add(tr, jax.ops.index[2:-2, 2:-2, :, vs.taup1], vs.dt_tracer * aloc[2:-2, 2:-2, :])

    """
    add implicit part
    """
    if iso:
        aloc = jax.ops.index_update(aloc, jax.ops.index[...],
            tr[:, :, :, vs.taup1])
    ...
    if vs.enable_conserve_energy:
        ...
        if not iso:
            vs.P_diss_skew = jax.ops.index_add(vs.P_diss_skew, jax.ops.index[2:-2, 2:-2, :-1],
                - vs.grav / vs.rho_0 * \
                fxa * vs.flux_top[2:-2, 2:-2, :-1] * vs.maskW[2:-2, 2:-2, :-1])
        else:
            vs.P_diss_iso = jax.ops.index_add(vs.P_diss_iso, jax.ops.index[2:-2, 2:-2, :-1],
                - vs.grav / vs.rho_0 * fxa * vs.flux_top[2:-2, 2:-2, :-1] * vs.maskW[2:-2, 2:-2, :-1] \
