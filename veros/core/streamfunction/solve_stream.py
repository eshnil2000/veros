...
def solve_streamfunction(vs):
    ...
    vs.p_hydro = jax.ops.index_update(vs.p_hydro, jax.ops.index[:, :, -1],
        tmp[:, :, -1])
    tmp = jax.ops.index_add(tmp, jax.ops.index[:, :, :-1],
        0.5 * vs.rho[:, :, 1:, vs.tau] * \
        fxa * vs.dzw[:-1] * vs.maskT[:, :, :-1]
    )
    vs.p_hydro = jax.ops.index_update(vs.p_hydro, jax.ops.index[:, :, -2::-1],
        vs.maskT[:, :, -2::-1] * \
        (vs.p_hydro[:, :, -1, np.newaxis] + np.cumsum(tmp[:, :, -2::-1], axis=2))
    )

    # add hydrostatic pressure gradient
    vs.du = jax.ops.index_add(vs.du, jax.ops.index[2:-2, 2:-2, :, vs.tau],
        -(vs.p_hydro[3:-1, 2:-2, :] - vs.p_hydro[2:-2, 2:-2, :]) \
        / (vs.cost[np.newaxis, 2:-2, np.newaxis] * vs.dxu[2:-2, np.newaxis, np.newaxis]) \
        * vs.maskU[2:-2, 2:-2, :]
    )
    vs.dv = jax.ops.index_add(vs.dv, jax.ops.index[2:-2, 2:-2, :, vs.tau],
    ...
    vs.dpsi = jax.ops.index_update(vs.dpsi, jax.ops.index[:, :, vs.taup1],
        2 * vs.dpsi[:, :, vs.tau] - vs.dpsi[:, :, vs.taum1]
    )
    ...
    if vs.nisle > 1:
        ...
        line_forc = jax.ops.index_update(line_forc, jax.ops.index[1:],
            utilities.line_integrals(vs, fpx[..., np.newaxis], fpy[..., np.newaxis], kind='same')[1:]
        )

        # calculate island integrals of interior streamfunction
        fpx = jax.ops.index_update(fpx, jax.ops.index[...], 0.)
        fpy = jax.ops.index_update(fpy, jax.ops.index[...], 0.)
        fpx = jax.ops.index_update(fpx, jax.ops.index[1:, 1:],
            -vs.maskU[1:, 1:, -1] \
            * (vs.dpsi[1:, 1:, vs.taup1] - vs.dpsi[1:, :-1, vs.taup1]) \
            / vs.dyt[np.newaxis, 1:] * vs.hur[1:, 1:]
        )
        fpy = jax.ops.index_update(fpy, jax.ops.index[1:, 1:],
            vs.maskV[1:, 1:, -1] \
            * (vs.dpsi[1:, 1:, vs.taup1] - vs.dpsi[:-1, 1:, vs.taup1]) \
            / (vs.cosu[np.newaxis, 1:] * vs.dxt[1:, np.newaxis]) * vs.hvr[1:, 1:]
        )
        line_forc = jax.ops.index_add(line_forc, jax.ops.index[1:],
            -utilities.line_integrals(vs, fpx[..., np.newaxis], fpy[..., np.newaxis], kind='same')[1:]
        )

        # solve for time dependent boundary values
        line_forc = jax.ops.index_update(line_forc, jax.ops.index[1:],
            np.linalg.solve(vs.line_psin[1:, 1:], line_forc[1:])
        )

        vs.dpsin = jax.ops.index_update(vs.dpsin, jax.ops.index[1:, vs.tau], line_forc[1:])

    # integrate barotropic and baroclinic velocity forward in time
    vs.psi = jax.ops.index_update(vs.psi, jax.ops.index[:, :, vs.taup1],
        vs.psi[:, :, vs.tau] + vs.dt_mom * ((1.5 + vs.AB_eps) * vs.dpsi[:, :, vs.taup1]
                                                               - (0.5 + vs.AB_eps) * vs.dpsi[:, :, vs.tau])
    )
    vs.psi = jax.ops.index_add(vs.psi, jax.ops.index[:, :, vs.taup1],
        vs.dt_mom * np.sum(((1.5 + vs.AB_eps) * vs.dpsin[1:, vs.tau]
                                                           - (0.5 + vs.AB_eps) * vs.dpsin[1:, vs.taum1]) * vs.psin[:, :, 1:], axis=2)
    )
    vs.u = jax.ops.index_update(vs.u, jax.ops.index[:, :, :, vs.taup1],
        vs.u[:, :, :, vs.tau] + vs.dt_mom * (vs.du_mix + (1.5 + vs.AB_eps) * vs.du[:, :, :, vs.tau]
                                                                             - (0.5 + vs.AB_eps) * vs.du[:, :, :, vs.taum1]) * vs.maskU
    )
    vs.v = jax.ops.index_update(vs.v, jax.ops.index[:, :, :, vs.taup1],
        vs.v[:, :, :, vs.tau] + vs.dt_mom * (vs.dv_mix + (1.5 + vs.AB_eps) * vs.dv[:, :, :, vs.tau]
    ...
    vs.u = jax.ops.index_add(vs.u, jax.ops.index[:, :, :, vs.taup1],
        -fpx[:, :, np.newaxis] * \
        vs.maskU * vs.hur[:, :, np.newaxis]
    )
    vs.v = jax.ops.index_add(vs.v, jax.ops.index[:, :, :, vs.taup1],
        -fpy[:, :, np.newaxis] * \
        vs.maskV * vs.hvr[:, :, np.newaxis]
    )

    # add barotropic mode to baroclinic velocity
    vs.u = jax.ops.index_add(vs.u, jax.ops.index[2:-2, 2:-2, :, vs.taup1],
        -vs.maskU[2:-2, 2:-2, :]\
    ...
    vs.v = jax.ops.index_add(vs.v, jax.ops.index[2:-2, 2:-2, :, vs.taup1],
        vs.maskV[2:-2, 2:-2, :]\
