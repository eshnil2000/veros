...
def thermodynamics(vs):
    ...
    if vs.enable_conserve_energy:
        ...
        if vs.enable_superbee_advection:
            advection.adv_flux_superbee(vs, vs.flux_east, vs.flux_north, vs.flux_top, vs.Hd[:, :, :, vs.tau])
        else:
            advection.adv_flux_2nd(vs, vs.flux_east, vs.flux_north, vs.flux_top, vs.Hd[:, :, :, vs.tau])

        vs.dHd = jax.ops.index_update(vs.dHd, jax.ops.index[2:-2, 2:-2, :, vs.tau],
            vs.maskT[2:-2, 2:-2, :] * (-(vs.flux_east[2:-2, 2:-2, :] - vs.flux_east[1:-3, 2:-2, :])
        ...
        vs.dHd = jax.ops.index_add(vs.dHd, jax.ops.index[:, :, 0, vs.tau],
            -vs.maskT[:, :, 0] \
            * vs.flux_top[:, :, 0] / vs.dzt[0]
        )
        vs.dHd = jax.ops.index_add(vs.dHd, jax.ops.index[:, :, 1:, vs.tau],
            -vs.maskT[:, :, 1:] \
        ...
        aloc = jax.ops.index_update(aloc, jax.ops.index[2:-2, 2:-2, :],
            vs.grav / vs.rho_0 * (-vs.int_drhodT[2:-2, 2:-2, :, vs.tau] * vs.dtemp[2:-2, 2:-2, :, vs.tau]
        ...
        """
        aloc = jax.ops.index_add(jax.ops.index[:, :, :-1],
            -0.25 * vs.grav / vs.rho_0 * vs.w[:, :, :-1, vs.tau] \
    ...
    if vs.enable_conserve_energy and vs.enable_tke:
        ...
        """
        vs.P_diss_adv = jax.ops.index_update(vs.P_diss_adv, jax.ops.index[...], 0.)
        ...
        vs.P_diss_adv = jax.ops.index_update(vs.P_diss_adv, jax.ops.index[...], 0.)
        vs.P_diss_adv = jax.ops.index_update(vs.P_diss_adv, jax.ops.index[2:-2, 2:-2, :-1], fxa / fxb * tke_mask)
        vs.P_diss_adv = jax.ops.index_update(vs.P_diss_adv, jax.ops.index[2:-2, 2:-2, -1], fxa / fxb)

    """
    Adam Bashforth time stepping for advection
    """
    vs.temp = jax.ops.index_update(vs.temp, jax.ops.index[:, :, :, vs.taup1],
        vs.temp[:, :, :, vs.tau] + vs.dt_tracer \
        * ((1.5 + vs.AB_eps) * vs.dtemp[:, :, :, vs.tau]
        - (0.5 + vs.AB_eps) * vs.dtemp[:, :, :, vs.taum1]) * vs.maskT
    )
    vs.salt = jax.ops.index_update(vs.salt, jax.ops.index[:, :, :, vs.taup1],
        vs.salt[:, :, :, vs.tau] + vs.dt_tracer \
    ...
    with vs.timers['isoneutral']:
        ...
        if vs.enable_neutral_diffusion:
            vs.P_diss_iso = jax.ops.index_update(vs.P_diss_iso, jax.ops.index[...], 0.0)
            vs.dtemp_iso = jax.ops.index_update(vs.dtemp_iso, jax.ops.index[...], 0.0)
            vs.dsalt_iso = jax.ops.index_update(vs.dsalt_iso, jax.ops.index[...], 0.0)
            ...
            if vs.enable_skew_diffusion:
                vs.P_diss_skew = jax.ops.index_update(vs.P_diss_skew, jax.ops.index[...], 0.0)
    ...
    with vs.timers['vmix']:
        ...
        """
        vs.dtemp_vmix = jax.ops.index_update(vs.dtemp_vmix, jax.ops.index[...], vs.temp[:, :, :, vs.taup1])
        vs.dsalt_vmix = jax.ops.index_update(vs.dsalt_vmix, jax.ops.index[...], vs.salt[:, :, :, vs.taup1])
        ...
        delta = jax.ops.index_update(delta, jax.ops.index[:, :, :-1],
            vs.dt_tracer / vs.dzw[np.newaxis, np.newaxis, :-1] \
            * vs.kappaH[2:-2, 2:-2, :-1]
        )
        delta = jax.ops.index_update(delta, jax.ops.index[:, :, -1], 0.)
        ...
        d_tri = jax.ops.index_update(d_tri, jax.ops.index[...],
            vs.temp[2:-2, 2:-2, :, vs.taup1])
        d_tri = jax.ops.index_add(d_tri, jax.ops.index[:, :, -1],
            vs.dt_tracer * vs.forc_temp_surface[2:-2, 2:-2] / vs.dzt[-1])
        sol, mask = utilities.solve_implicit(vs, ks, a_tri, b_tri, c_tri, d_tri, b_edge=b_tri_edge)
        vs.temp = jax.ops.index_update(vs.temp, jax.ops.index[2:-2, 2:-2, :, vs.taup1],
            utilities.where(vs, mask, sol, vs.temp[2:-2, 2:-2, :, vs.taup1]))
        d_tri = jax.ops.index_update(d_tri, jax.ops.index[...],
            vs.salt[2:-2, 2:-2, :, vs.taup1])
        d_tri = jax.ops.index_add(d_tri, jax.ops.index[:, :, -1],
            vs.dt_tracer * vs.forc_salt_surface[2:-2, 2:-2] / vs.dzt[-1])
        sol, mask = utilities.solve_implicit(
            vs, ks, a_tri, b_tri, c_tri, d_tri, b_edge=b_tri_edge
        )
        vs.salt = jax.ops.index_update(vs.salt, jax.ops.index[2:-2, 2:-2, :, vs.taup1],
            utilities.where(vs, mask, sol, vs.salt[2:-2, 2:-2, :, vs.taup1]))

        vs.dtemp_vmix = jax.ops.index_update(vs.dtemp_vmix, jax.ops.index[...],
            (vs.temp[:, :, :, vs.taup1] - vs.dtemp_vmix) / vs.dt_tracer)
        vs.dsalt_vmix = jax.ops.index_update(vs.dsalt_vmix, jax.ops.index[...],
            (vs.salt[:, :, :, vs.taup1] - vs.dsalt_vmix) / vs.dt_tracer)
    ...
    """
    vs.forc_rho_surface = jax.ops.index_update(vs.forc_rho_surface, jax.ops.index[...],
        vs.maskT[:, :, -1] * (
    ...
    with vs.timers['vmix']:
        vs.P_diss_v = jax.ops.index_update(vs.P_diss_v, jax.ops.index[...], 0.0)
        if vs.enable_conserve_energy:
            """
            ...
            vs.P_diss_v = jax.ops.index_add(vs.P_diss_v, jax.ops.index[2:-2, 2:-2, :-1],
                -vs.grav / vs.rho_0 * fxa * vs.kappaH[2:-2, 2:-2, :-1] \
        ...
        if vs.enable_conserve_energy:
            ...
            """
            aloc = jax.ops.index_update(aloc, jax.ops.index[:, :, :-1],
                vs.kappaH[:, :, :-1] * vs.Nsqr[:, :, :-1, vs.taup1])
            vs.P_diss_nonlin = jax.ops.index_update(vs.P_diss_nonlin, jax.ops.index[:, :, :-1],
                vs.P_diss_v[:, :, :-1] - aloc[:, :, :-1])
            vs.P_diss_v = jax.ops.index_update(vs.P_diss_v, jax.ops.index[:, :, :-1],
                aloc[:, :, :-1])
        else:
            """
            ...
            vs.P_diss_v = jax.ops.index_update(vs.P_diss_v, jax.ops.index[:, :, :-1],
                vs.kappaH[:, :, :-1] * vs.Nsqr[:, :, :-1, vs.taup1])
            vs.P_diss_v = jax.ops.index_update(vs.P_diss_v, jax.ops.index[:, :, -1],
                -vs.forc_rho_surface * vs.maskT[:, :, -1] * vs.grav / vs.rho_0)
...
def advect_tracer(vs, tr, dtr):
    ...
    dtr = jax.ops.index_update(dtr, jax.ops.index[2:-2, 2:-2, :],
        vs.maskT[2:-2, 2:-2, :] * (-(vs.flux_east[2:-2, 2:-2, :] - vs.flux_east[1:-3, 2:-2, :])
                                                    / (vs.cost[np.newaxis, 2:-2, np.newaxis] * vs.dxt[2:-2, np.newaxis, np.newaxis])
                                                   - (vs.flux_north[2:-2, 2:-2, :] - vs.flux_north[2:-2, 1:-3, :])
                                                    / (vs.cost[np.newaxis, 2:-2, np.newaxis] * vs.dyt[np.newaxis, 2:-2, np.newaxis]))
    )
    dtr = jax.ops.index_add(dtr, jax.ops.index[:, :, 0], -vs.maskT[:, :, 0] * vs.flux_top[:, :, 0] / vs.dzt[0])
    dtr = jax.ops.index_add(dtr, jax.ops.index[:, :, 1:], -vs.maskT[:, :, 1:] * (vs.flux_top[:, :, 1:] - vs.flux_top[:, :, :-1]) / vs.dzt[1:])
...
def calc_eq_of_state(vs, n):
    ...
    """
    vs.rho = jax.ops.index_update(vs.rho, jax.ops.index[..., n],
        density.get_rho(vs, salt, temp, press) * vs.maskT)

    """
    calculate new potential density
    """
    vs.prho = jax.ops.index_update(vs.prho, jax.ops.index[...],
        density.get_potential_rho(vs, salt, temp) * vs.maskT)

    """
    calculate new dynamic enthalpy and derivatives
    """
    if vs.enable_conserve_energy:
        vs.Hd = jax.ops.index_update(vs.Hd, jax.ops.index[..., n],
            density.get_dyn_enthalpy(vs, salt, temp, press) * vs.maskT)
        vs.int_drhodT = jax.ops.index_update(vs.int_drhodT, jax.ops.index[..., n],
            density.get_int_drhodT(vs, salt, temp, press))
        vs.int_drhodS = jax.ops.index_update(vs.int_drhodS, jax.ops.index[..., n],
            density.get_int_drhodS(vs, salt, temp, press))

    """
    new stability frequency
    """
    fxa = -vs.grav / vs.rho_0 / vs.dzw[np.newaxis, np.newaxis, :-1] * vs.maskW[:, :, :-1]
    vs.Nsqr = jax.ops.index_update(vs.Nsqr, jax.ops.index[:, :, :-1, n],
        fxa * (density.get_rho(
