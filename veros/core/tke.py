...
def set_tke_diffusivities(vs):
    ...
    if vs.enable_tke:
        vs.sqrttke = jax.ops.index_update(vs.sqrttke, jax.ops.index[...],
            np.sqrt(np.maximum(0., vs.tke[:, :, :, vs.tau])))
        """
        calculate buoyancy length scale
        """
        vs.mxl = jax.ops.index_update(vs.mxl, jax.ops.index[...],
            math.sqrt(2) * vs.sqrttke \
        ...
        if vs.tke_mxl_choice == 1:
            ...
            vs.mxl = jax.ops.index_update(vs.mxl, jax.ops.index[...], np.maximum(vs.mxl, vs.mxl_min))
            ...
            for k in range(vs.nz - 2, -1, -1):
                vs.mxl = jax.ops.index_update(vs.mxl, jax.ops.index[:, :, k],
                    np.minimum(vs.mxl[:, :, k], vs.mxl[:, :, k + 1] + vs.dzt[k + 1])
                )
            vs.mxl = jax.ops.index_update(vs.mxl, jax.ops.index[:, :, -1],
                np.minimum(vs.mxl[:, :, -1], vs.mxl_min + vs.dzt[-1]))
            for k in range(1, vs.nz):
                vs.mxl = jax.ops.index_update(vs.mxl, jax.ops.index[:, :, k],
                    np.minimum(vs.mxl[:, :, k], vs.mxl[:, :, k - 1] + vs.dzt[k]))
            vs.mxl = jax.ops.index_update(vs.mxl, jax.ops.index[...],
                np.maximum(vs.mxl, vs.mxl_min))
        else:
        ...
        vs.kappaM = jax.ops.index_update(vs.kappaM, jax.ops.index[...],
            np.minimum(vs.kappaM_max, vs.c_k * vs.mxl * vs.sqrttke))
        Rinumber = jax.ops.index_update(Rinumber, jax.ops.index[...],
            vs.Nsqr[:, :, :, vs.tau] / \
            np.maximum(vs.K_diss_v / np.maximum(1e-12, vs.kappaM), 1e-12)
        )
        if vs.enable_idemix:
            Rinumber = jax.ops.index_update(Rinumber, jax.ops.index[...],
                np.minimum(Rinumber, vs.kappaM * vs.Nsqr[:, :, :, vs.tau]
                / np.maximum(1e-12, vs.alpha_c * vs.E_iw[:, :, :, vs.tau]**2))
            )
        if vs.enable_Prandtl_tke:
            vs.Prandtlnumber = jax.ops.index_update(vs.Prandtlnumber, jax.ops.index[...],
                np.maximum(1., np.minimum(10, 6.6 * Rinumber)))
        else:
            vs.Prandtlnumber = jax.ops.index_update(vs.Prandtlnumber, jax.ops.index[...], vs.Prandtl_tke0)
        vs.kappaH = jax.ops.index_update(vs.kappaH, jax.ops.index[...],
            np.maximum(vs.kappaH_min, vs.kappaM / vs.Prandtlnumber))
        if vs.enable_kappaH_profile:
            ...
            vs.kappaH = jax.ops.index_update(vs.kappaH, jax.ops.index[...],
                np.maximum(vs.kappaH, (0.8 + 1.05 / np.pi
                * np.arctan((-vs.zw[np.newaxis, np.newaxis, :] - 2500.)
                / 222.2)) * 1e-4)
            )
        vs.kappaM = jax.ops.index_update(vs.kappaM, jax.ops.index[...], np.maximum(vs.kappaM_min, vs.kappaM))
    else:
        vs.kappaM = jax.ops.index_update(vs.kappaM, jax.ops.index[...], vs.kappaM_0)
        vs.kappaH = jax.ops.index_update(vs.kappaH, jax.ops.index[...], vs.kappaH_0)
...
def integrate_tke(vs):
    ...
    if not vs.enable_store_cabbeling_heat:
        forc = jax.ops.index_add(forc, jax.ops.index[...], -vs.P_diss_nonlin)

    """
    transfer part of dissipation of EKE to TKE
    """
    if vs.enable_eke:
        forc = jax.ops.index_add(forc, jax.ops.index[...], vs.eke_diss_tke)

    if vs.enable_idemix:
        """
        transfer dissipation of internal waves to TKE
        """
        forc = jax.ops.index_add(forc, jax.ops.index[...], vs.iw_diss)
        """
        store bottom friction either in TKE or internal waves
        """
        if vs.enable_store_bottom_friction_tke:
            forc = jax.ops.index_add(forc, jax.ops.index[...], vs.K_diss_bot)
    else:  # short-cut without idemix
        if vs.enable_eke:
            forc = jax.ops.index_add(forc, jax.ops.index[...], vs.eke_diss_iw)
        else:  # and without EKE model
            if vs.enable_store_cabbeling_heat:
                forc = jax.ops.index_add(forc, jax.ops.index[...], vs.K_diss_gm + vs.K_diss_h - vs.P_diss_skew \
    ...
    d_tri = jax.ops.index_update(d_tri, jax.ops.index[...],
        vs.tke[2:-2, 2:-2, :, vs.tau] + dt_tke * forc[2:-2, 2:-2, :])
    d_tri = jax.ops_index_add(d_tri, jax.ops.index[:, :, -1],
        dt_tke * vs.forc_tke_surface[2:-2, 2:-2] / (0.5 * vs.dzw[-1]))

    sol, water_mask = utilities.solve_implicit(vs, ks, a_tri, b_tri, c_tri, d_tri, b_edge=b_tri_edge)
    vs.tke = jax.ops.index_update(vs.tke, jax.ops.index[2:-2, 2:-2, :, vs.taup1],
        utilities.where(vs, water_mask, sol, vs.tke[2:-2, 2:-2, :, vs.taup1]))

    """
    store tke dissipation for diagnostics
    """
    vs.tke_diss = jax.ops.index_update(vs.tke_diss, jax.ops.index[...],
        vs.c_eps / vs.mxl * vs.sqrttke * vs.tke[:, :, :, vs.taup1])

    """
    Add TKE if surface density flux drains TKE in uppermost box
    """
    mask = vs.tke[2:-2, 2:-2, -1, vs.taup1] < 0.0
    vs.tke_surf_corr = jax.ops.index_update(vs.tke_surf_corr, jax.ops.index[...], 0.)
    vs.tke_surf_corr = jax.ops.index_update(vs.tke_surf_corr, jax.ops.index[2:-2, 2:-2],
        utilities.where(vs, mask,
                        -vs.tke[2:-2, 2:-2, -1, vs.taup1] * 0.5
                            * vs.dzw[-1] / dt_tke,
                        0.)
    )
    vs.tke = jax.ops.index_update(vs.tke, jax.ops.index[2:-2, 2:-2, -1, vs.taup1],
        np.maximum(0., vs.tke[2:-2, 2:-2, -1, vs.taup1])
    )
    ...
    if vs.enable_tke_hor_diffusion:
        ...
        vs.flux_east = jax.ops.index_update(vs.flux_east, jax.ops.index[:-1, :, :],
            vs.K_h_tke * (vs.tke[1:, :, :, vs.tau] - vs.tke[:-1, :, :, vs.tau]) \
            / (vs.cost[np.newaxis, :, np.newaxis] * vs.dxu[:-1, np.newaxis, np.newaxis]) * vs.maskU[:-1, :, :]
        )
        if vs.pyom_compatibility_mode:
            vs.flux_east = jax.ops.index_update(vs.flux_east, jax.ops.index[-5, :, :], 0.)
        else:
        ...
        vs.tke = jax.ops.index_add(vs.tke, jax.ops.index[2:-2, 2:-2, :, vs.taup1],
            dt_tke * vs.maskW[2:-2, 2:-2, :] * \
    ...
    if vs.enable_tke_superbee_advection or vs.enable_tke_upwind_advection:
        vs.dtke = jax.ops.index_update(vs.dtke, jax.ops.index[2:-2, 2:-2, :, vs.tau],
            vs.maskW[2:-2, 2:-2, :] * (-(vs.flux_east[2:-2, 2:-2, :] - vs.flux_east[1:-3, 2:-2, :])
                / (vs.cost[np.newaxis, 2:-2, np.newaxis] * vs.dxt[2:-2, np.newaxis, np.newaxis])
            - (vs.flux_north[2:-2, 2:-2, :] - vs.flux_north[2:-2, 1:-3, :])
                / (vs.cost[np.newaxis, 2:-2, np.newaxis] * vs.dyt[np.newaxis, 2:-2, np.newaxis]))
        )
        vs.dtke = jax.ops.index_add(vs.dtke, jax.ops.index[:, :, 0, vs.tau], -vs.flux_top[:, :, 0] / vs.dzw[0])
        vs.dtke = jax.ops.index_add(vs.dtke, jax.ops.index[:, :, 1:-1, vs.tau], -(vs.flux_top[:, :, 1:-1] - vs.flux_top[:, :, :-2]) / vs.dzw[1:-1])
        vs.dtke = jax.ops.index_add(vs.dtke, jax.ops.index[:, :, -1, vs.tau], -(vs.flux_top[:, :, -1] - vs.flux_top[:, :, -2]) / (0.5 * vs.dzw[-1]))
        """
        Adam Bashforth time stepping
        """
        vs.tke = jax.ops.index_add(vs.tke, jax.ops.index[:, :, :, vs.taup1],
            vs.dt_tracer * ((1.5 + vs.AB_eps) * vs.dtke[:, :, :, vs.tau]
