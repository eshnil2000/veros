...
class GlobalOneDegreeSetup(VerosSetup):
    ...
    def set_topography(self, vs):
        ...
        vs.kbot = jax.ops.index_update(vs.kbot, jax.ops.index[2:-2, 2:-2][mask_channel],
        0
    ...
    def set_initial_conditions(self, vs):
        ...
        if vs.enable_idemix:
            ...
            mask = np.maximum(0, vs.kbot = jax.ops.index_update(vs.kbot, jax.ops.index[2:-2, 2:-2] - 1)[:, :, np.newaxis],
       = np.arange(vs.nz)[np.newaxis, np.newaxis, :]
    ...
    def set_forcing(self, vs):
        ...
        vs.surface_taux = jax.ops.index_update(vs.surface_taux, jax.ops.index[:-1, :],
        f1 * vs.taux[1:, :, n1] + f2 * vs.taux[1:, :, n2]
        vs.surface_tauy = jax.ops.index_update(vs.surface_tauy, jax.ops.index[:, :-1],
        f1 * vs.tauy[:, 1:, n1] + f2 * vs.tauy[:, 1:, n2]
        ...
        (vs.qqnet + vs.qqnec * (t_star_cur - vs.temp[..., -1, vs.tau])) \
